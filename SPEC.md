...
| `ConvertFrom-SsmRegistrationJson -Json` | object `{ ManagedInstanceId, Region }`; throws on malformed JSON / missing `ManagedInstanceID` / `ManagedInstanceID` not matching `^mi-[a-f0-9]{17}$` (source key is `ManagedInstanceID`, exposed as `ManagedInstanceId`) |
